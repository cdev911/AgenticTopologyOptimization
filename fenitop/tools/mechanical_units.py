"""Shared deterministic mechanical-unit validation and normalization.

Pint stays behind JSON-safe Pydantic models. No Pint quantity crosses the
formulation, public-tool, approval, or solver boundaries.
"""
...
def _require_dimension(unit_name: str, quantity_kind: QuantityKind) -> None:
    ...
    if candidate.dimensionality != expected.dimensionality:
        raise ValueError(f"{unit_name!r} is not a {quantity_kind} unit.")
...
def resultant_to_traction(
    resultant: tuple[int | float, int | float],
    *,
    boundary_measure: float,
    context: MechanicalUnitContext,
) -> tuple[tuple[float, float], tuple[float, float]]:
    """Convert force to stress and verify the integrated force round trip."""
    measure = float(boundary_measure)
    if not math.isfinite(measure) or measure <= 0:
        raise ValueError("boundary_measure must be finite and positive.")
    area_unit = _unit(context.length_unit) ** 2
    force_unit = _unit(context.force_unit)
    stress_unit = _unit(context.stress_unit)
    area_value = measure * context.thickness_value
    traction = tuple(
        float(
            (float(component) * force_unit / (area_value * area_unit))
            .to(stress_unit)
            .magnitude
        )
    ...
    integrated = tuple(
        float(
            (float(component) * stress_unit * area_value * area_unit)
            .to(force_unit)
            .magnitude
        )
        for component in traction
    )
    return traction, integrated


def traction_to_resultant(
    traction: tuple[int | float, int | float],
    *,
    boundary_measure: float,
    context: MechanicalUnitContext,
) -> tuple[float, float]:
    """Integrate stress over resolved boundary length and unit thickness."""
    measure = float(boundary_measure)
    if not math.isfinite(measure) or measure <= 0:
        raise ValueError("boundary_measure must be finite and positive.")
    area_unit = _unit(context.length_unit) ** 2
